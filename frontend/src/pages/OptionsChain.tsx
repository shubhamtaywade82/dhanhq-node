...
export function OptionsChain() {
  ...
  const spot = (idx as IndexData)?.ltp || 0;
  ...
  useEffect(() => {
    if (oiRef.current) drawOiChart(oiRef.current, chartRows, atm);
    if (skewRef.current) drawIvSkewChart(skewRef.current, chartRows);
  }, [chartRows, atm]);
      ...
      <Card className="p-3.5 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div className="text-xs font-mono text-muted pl-2 border-l border-border">
            ATM: <span className="text-gold font-bold">{atm > 0 ? fmt(atm, 0) : '—'}</span>
          </div>
        </div>
        <div className="flex items-center gap-4 text-xs font-mono text-muted">
          <div>Spot: <span className="text-white font-semibold">{spot > 0 ? fmt(spot) : '—'}</span></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card className="p-3.5">
          <div className="text-[9px] font-mono text-muted uppercase tracking-widest mb-2 font-semibold flex items-center justify-between">
            ...
            <span className="text-gold text-[9px]">ATM Strike: {atm > 0 ? fmt(atm, 0) : '—'}</span>
          </div>
...
function ChainRow({ row, isATM, maxOi, rowRef }: { row: OptionStrikeRow; isATM: boolean; maxOi: number; rowRef?: RefObject<HTMLTableRowElement | null> }) {
  ...
  const ceCls = 'text-right px-2.5 py-[7px] border-b border-border/60 text-accent/80 font-mono text-[10px]';
  const peCls = 'text-left px-2.5 py-[7px] border-b border-border/60 text-danger/80 font-mono text-[10px]';
  return (
    <tr ref={rowRef} className={isATM ? 'bg-gold/5' : ''}>
      ...
      <td className={ceCls}>{fmt(row.ce.volume, 0)}</td>
      <td className={ceCls}>{row.ce.iv ? `${fmt(row.ce.iv, 1)}%` : '—'}</td>
      <td className={ceCls}>{row.ce.delta || '—'}</td>
      ...
      <td className={peCls}>{row.pe.delta || '—'}</td>
      <td className={peCls}>{row.pe.iv ? `${fmt(row.pe.iv, 1)}%` : '—'}</td>
      <td className={peCls}>{fmt(row.pe.volume, 0)}</td>
...
function drawOiChart(canvas: HTMLCanvasElement, rows: OptionStrikeRow[], atm: number) {
  ...
  if (rows.length === 0) {
    ...
    ctx.fillText('No open interest data available', mid, h / 2 + 4);
    return;
  }
  ...
  rows.forEach((r, i) => {
    ...
    ctx.fillStyle = isAtm ? '#f0b429' : 'rgba(0, 229, 160, 0.4)';
    ctx.fillRect(mid - 15 - ceW, y, ceW, 6);
    ctx.fillStyle = isAtm ? '#f0b429' : 'rgba(255, 59, 92, 0.4)';
    ctx.fillRect(mid + 15, y, peW, 6);
    ctx.font = '8px JetBrains Mono';
    ctx.fillStyle = isAtm ? '#f0b429' : '#64748b';
    ctx.textAlign = 'center';
    ctx.fillText(isAtm ? 'ATM' : String(r.strike), mid, y + 5.5);
  });
}
...
function drawIvSkewChart(canvas: HTMLCanvasElement, rows: OptionStrikeRow[]) {
  ...
  if (rows.length === 0) {
    ...
    ctx.fillStyle = '#64748b';
    ctx.textAlign = 'center';
    ctx.fillText('No IV data available', w / 2, h / 2 + 4);
    return;
  }
