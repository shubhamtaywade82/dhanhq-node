        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-bold tracking-wider text-white flex items-center gap-2">
              ...
              <span className="text-[9px] font-mono bg-accent/10 text-accent border border-accent/20 px-1.5 py-0.5 rounded">v4.2 PROD</span>
            </div>
            <div className="text-[9.5px] font-mono text-muted tracking-wide">Options Engine & Multi-Agent Control Plane</div>
          </div>
        </div>
        <div className="h-6 w-px bg-border mx-2" />
        <div className="flex items-center gap-2">
          <h1 className="text-xs font-semibold text-white">{pageTitle}</h1>
          <span className="text-[9px] font-mono text-muted bg-surface-300 px-2 py-0.5 rounded border border-border">{pageSubtitle}</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-5">
        <div className="hidden lg:flex items-center gap-4 font-mono text-xs">
          <div className="flex items-center gap-1.5">
            ...
            <span className={`text-[10px] ${nifty.change >= 0 ? 'text-accent' : 'text-danger'}`}>
              {nifty.change >= 0 ? '+' : ''}{fmt(nifty.pct)}%
            </span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="text-muted text-[10px]">BANKNIFTY</span>
            <span className="text-white font-semibold">{fmt(bnf.ltp)}</span>
            <span className={`text-[10px] ${bnf.change >= 0 ? 'text-accent' : 'text-danger'}`}>
              {bnf.change >= 0 ? '+' : ''}{fmt(bnf.pct)}%
            </span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="text-muted text-[10px]">INDIA VIX</span>
            <span className="text-gold font-semibold">{fmt(vix.ltp)}</span>
            <span className="text-accent text-[10px]">{vix.change >= 0 ? '+' : ''}{fmt(vix.pct)}%</span>
          </div>
        </div>
